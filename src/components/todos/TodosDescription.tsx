...
export const TodosDescription = () => {
  ...
  return (
    <div onClick={() => setIsExpanded(!isExpanded)} className={`absolute cursor-pointer -top-2 z-20 right-0 p-1 bg-white  rounded-xl transition-all duration-500 ease-in-out transform  ${
        isExpanded ? 'w-full max-h-96 border-4 border-gray-400 opacity-95 scale-100 rotate-0 md:w-96' : 'w-32 max-h-12 scale-80'
      }`}>
      <div className="flex justify-between items-center">
        ...
        >
          <FontAwesomeIcon icon={isExpanded ? faAngleUp : faAngleDown} size="sm"/>
        </button>
      </div>
        ...
        <div className="flex flex-col gap-2 text-gray-700">
          ...
          <section className="border-t">
            ...
            <p className="text-sm">
              ...
              曜日ボタンを押すと、選択した曜日になると✓が外れた状態に戻ります。繰り返しのタスクに便利です。
            </p>
          </section>
        </div>
      </div>
